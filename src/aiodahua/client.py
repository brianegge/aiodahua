...
def _parse_adts_frames(data: bytes) -> list[bytes]:
    ...
    Cameras want each AAC frame as its own MIME part so the decoder always
    receives a complete, decodable unit.
    """
    frames: list[bytes] = []
    offset = 0
...
class DahuaClient:
    ...
    def _normalise(endpoint: str) -> str:
        """Accept ``foo.cgi?x=1``, ``/cgi-bin/foo.cgi?x=1`` or ``cgi-bin/foo.cgi``."""
        endpoint = endpoint.lstrip("/")
        if endpoint.startswith("cgi-bin/"):
            endpoint = endpoint[len("cgi-bin/") :]
        return endpoint
    ...
    async def async_get_text(self, endpoint: str) -> str:
        ...
        """
        endpoint = self._normalise(endpoint)
    ...
    # Fork-compatible transport aliases. The methods carried over from the Home
    ...
    async def get_bytes(self, url: str) -> bytes:
        ...
        return await self.async_get_bytes(url)

    @staticmethod
    async def parse_dahua_api_response(data: str) -> dict[str, str]:
        """Parse a ``key=value`` body. Kept async for drop-in compatibility."""
    ...
    # ------------------------------------------------------------------
    # Carried over from the Dahua Home Assistant integration (rroller/dahua,
    # MIT). Behaviour is intentionally unchanged so the integration can adopt
    # this library without a semantic diff.
    # ------------------------------------------------------------------
    ...
    async def async_set_zoom_v1(self, zoom: float):
        ...
        url = f"/cgi-bin/devVideoInput.cgi?action=adjustFocuszoom={zoom!s}"
        return await self.get(url, True)
    ...
    async def async_set_focus_v1(self, focus: float):
        ...
        url = f"/cgi-bin/devVideoInput.cgi?action=adjustFocus&focus={focus!s}"
        return await self.get(url, True)
    ...
    async def async_set_service_set_channel_title(
        ...
        url = "/cgi-bin/configManager.cgi?action=setConfig&" + build_config_query(
            {f"ChannelTitle[{channel}].Name": text}
        )
        value = await self.get(url)
        if "OK" not in value and "ok" not in value:
